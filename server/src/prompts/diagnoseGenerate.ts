...
import type { Platform, BrandTone, InputLanguage, ReferenceCase, CalendarEvent } from '../types/index.js';
...
function getPlatformInstructions(platform: Platform): string {
  switch (platform) {
    ...
    case 'shorts':
      return '只生成 YouTube Shorts 版本作為主要輸出，其他版本可以簡短但必須提供';
    case 'all':
      return '全部平台（IG + Facebook + YouTube Shorts），每個版本都要完整';
  }
}
...
- 🔥 **加 hook**：每個版本開頭加一句搶眼嘅廣東話 hook（尤其是 Shorts 同 IG 版本）
...
- 加入適當嘅粵語語氣詞同港式表達
- 根據平台需要調整內容深淺（IG 輕巧啲、FB 多啲背景、Shorts 有節奏感）
- 字數浮動唔超過 ±50%
...
- 可以考慮將話題角度同平台特性結合：IG 用打卡/視覺角度、Facebook 用長文討論角度、Shorts 用快節奏 hook
...
### 創作參數
- 目標平台：${platform === 'all' ? 'IG + Facebook + YouTube Shorts' : platform}
- 品牌語氣：${tone}
...
- Shorts 版本：必須有強 hook + 口語節奏 + punch line，頭 3 秒就要搶眼——可以係對白、可以係詩歌節奏、可以係快問快答
