...
执行项目管理/07-ClaudeCode-Slice-B-真实Auth接入执行单.md。用户已批准 Agent Teams 和 YOLO/bypass-permissions。只完成 Supabase 真实 Auth 应用接线与本地严格验证；使用 Publishable key + 用户 JWT/RLS，不使用 Secret/service_role，不再迁移数据库，不进入 Slice C、支付或部署。需要真实邮箱收信或创建测试账户时停止并汇报。
