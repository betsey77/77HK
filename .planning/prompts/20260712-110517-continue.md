...
Generated: 2026-07-12T11:05:17
Project: D:\work\77港话通社媒文案\77

## User Objective

完成工作台 UX-F1 切片：实现明确标注为预估的四阶段生成进度（诊断原文→生成变体→质量审核→消费者反馈），并收纳 Header 右侧低频功能。保留历史、收藏与紧凑引擎状态；将官网、复原配置、主题切换、账号邮箱和退出放入可访问的账户/更多菜单。先写失败行为测试再实现，桌面端为主，亮色橙色/暗色荧光绿，遵循 docs/design-system.md。修复后保留参考收藏案例入口始终可见。不得修改数据库、支付、Supabase Migration、真实微信通知或任何 secret。完成全量 client tests、tsc、build、截图/浏览器证据，并更新 PRD/SDD/TEST_PLAN/ACCEPTANCE/CHANGELOG/planning/evidence。完成当前切片后停止并汇报。
