...
Generated: 2026-07-12T14:05:34
Project: D:\work\77港话通社媒文案\77

## User Objective

完成 Slice H1「用户反馈中心 + Server酱通知本地闭环 + 收藏删除防误触」，严格 TDD，一次只做本切片，完成后停止。

用户已明确选择 Server酱 Turbo，并授权从外部文件 D:\API\微信推送\sendkey.txt 读取 SendKey。严禁打印、复制、提交、写入日志或错误消息；仓库和前端不得出现 SendKey。可在全部本地测试通过后发送且仅发送 1 条不含用户数据的测试消息，标题明确为「77港话通开发测试」，测试结果必须脱敏。配置采用 SERVERCHAN_SENDKEY_FILE 指针，支持文件中为 raw key 或 SERVERCHAN_SENDKEY=...，直接 env 优先；URL/异常必须脱敏。

范围：
1) 收藏库删除必须先弹出符合现有 shadcn-like 规范的可访问确认对话框，显示文案摘要；取消不删除，确认才触发现有云同步删除。支持 Escape、焦点、aria，亮色橙色/暗色荧光绿，但危险确认用红色。
2) HeaderMenu 增加「意见反馈」入口，打开 FeedbackCenter drawer/dialog。类型：需求建议、Bug反馈、使用体验、其他；标题与内容必填并有长度限制；自动附带 page path 与 app version，不采集完整 URL query、token 或额外敏感浏览器信息。提供提交 loading/success/error、空状态和最近自己的反馈列表；桌面优先。
3) Server 新增受 requireAuth 保护的 POST/GET /api/feedback；严格输入校验、长度限制、错误脱敏。反馈必须先成功持久化，再 best-effort 通知 Server酱；通知失败不得回滚或让反馈丢失，记录 pending/sent/failed、attempts、last_error(脱敏)、notified_at。不要把反馈正文写入普通 server log。
4) 起草但绝不推送 Supabase Migration 20260712..._slice_h1_user_feedback.sql：user_feedback 表、owner RLS、普通用户只能 insert/select 自己且不能修改通知/状态字段、admin/super_admin 可读取、service_role 可更新通知状态；必要索引、长度/check、updated_at、安全 grant/revoke、简单防滥用限制。Migration 版本与云端 history/dry-run 结果记录在 docs，但禁止 db push。
5) ServerChan notifier 必须可注入/可 mock，超时与网络失败有测试；成功响应也校验 errno/code。外部密钥文件读取使用现有 secret loader 风格，路径只可在 .env.example 和本地 server/.env 指针中出现，绝不读取后回显。
6) 更新 PRD/SDD/TEST_PLAN/ACCEPTANCE/CHANGELOG、.planning status/progress/task_plan/context、docs/evidence；记录云端版本：当前远端最新 20260712070000，H1 Migration 本地待推送。不得改支付、额度、生成流程、F1 进度、生产部署。
7) 先写失败测试再实现；覆盖确认删除、表单校验、成功/失败、未登录、跨用户隔离静态/数据库断言、通知失败仍 201、密钥不泄漏。跑 client/server 全量 tests、双端 tsc/build。UI 保存桌面截图或至少浏览器可核验路径与严格说明。
8) 真实数据库 Migration 推送必须停止等待用户再次明确确认；不允许自动 push。
